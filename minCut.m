function final_patch = minCut(ref_patches, selected_patch, overlap_size, overlap_type, patch_size)
%   initializing the final patch with zeros
...
%       finding the edge capacities by taking the sum of absolutes
%       differences between the left overlap and selected overlap patches 
%       here e is the adjacency matrix with weights.
        diff_patch = left_overlap - selected_overlap;
        e = sum(abs(diff_patch),3);
        E = zeros(size(e));
        disp('Vertical weighted  Adjacency matrix');
        disp(e);
        disp(length(e));
        E(1,:) = e(1,:);
%       choosing the edges based on the minimum cost function mentioned in
%       the paper
        for i = 2:patch_size
...
%       here e is the adjacency matrix with weights.
		...
		E = zeros(size(e));
        disp('Horizontal weighted adjacency matrix');
        disp(e);
%       choosing the edges based on the minimum cost function mentioned in
%       the paper
        E(:,1) = e(:,1);  
...
%         
        horizontal_cut_patch = minCut(ref_patches,selected_patch,overlap_size,'horizontal',patch_size);
		final_patch = minCut(ref_patches,horizontal_cut_patch,overlap_size,'vertical',patch_size);
    end		
end